...
class InventoryItem(models.Model):
    ...
    price_rub = models.DecimalField(max_digits=10, decimal_places=2, verbose_name="Цена (руб)")
...
class ShoppingCartItem(models.Model):
    ...
    def __str__(self):
        return f"{self.item.name} × {self.quantity} (в корзине {self.cart.user.username})"
...
class PurchaseOrder(models.Model):
    ...
    total_rub = models.DecimalField(max_digits=10, decimal_places=2, verbose_name="Сумма заказа (руб)")
...
class OrderItem(models.Model):
    ...
    item = models.ForeignKey(InventoryItem, on_delete=models.PROTECT, verbose_name="Товар")
    quantity = models.PositiveIntegerField(verbose_name="Количество")
    price_at_order_rub = models.DecimalField(max_digits=10, decimal_places=2, verbose_name="Цена на момент заказа")
